...
class MonitoringDZ(object):
		...
		try:
			...
			with open(self.path+'config.ini','r',encoding='utf8') as f:
				for line in f.readlines():
					...
					if '=' in line and class_ != 'site':
						conf = line.split('=')
						configs[conf[0].strip()] = conf[1].strip()

					if class_ == 'site' and line != '[site]':
						site =  line.split(',')
						config = {}
						for site_ in site:
							site_ = site_.strip()
	...
	def putHtml(self,html):
		...
		fhtml = open(self.path+'html/'+self.local.url.replace('http://','').replace('https://','')+'.html','w',encoding='utf8')
		fhtml.write(html)
	...
	def main(self):
		...
		if not configs:
			...
			joinlist = []
			global configcount
			configcount = len(configs['config'])
			for c in configs['config']:
				print('正在启动 ',c['url'],' 站点...')
				myqueue.put(c['url'])
