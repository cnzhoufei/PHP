...
class MonitoringDZ(object):
	...
	def validation_(self,url):
		...
		root.geometry('500x200+200+200')#设置窗口大小和位置
		text = tkinter.Label(root, text=url, font=("Arial",15), width=100, height=3)#显示文本
		text.pack()
		text = tkinter.Label(root, text="请确认验证码验证成功后再点击我也手动验证验证码", bg="#fff",fg='#f00', font=("Arial",13), width=100, height=3)#显示文本
		text.pack()
		def des():
			root.destroy()
		button = tkinter.Button(root,text='我也手动验证验证码',command=des,width=30,height=2)
		button.pack()
		root.mainloop()
		return 1
