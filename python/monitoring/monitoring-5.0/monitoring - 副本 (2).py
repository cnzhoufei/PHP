...
class Monitoring(object):
	...
	def login(self,url,username,password,msg,loginnum=1):
		...
		try:
			print(url ,' 正在登录...')
			#访问一次登录页面
			getresponse = self.conn.get(url+'/member.php?mod=logging&action=login',headers=self.headers)
			...
			data = {
					'formhash':formhash,
					'referer':url+'/home.php?mod=space&do=friend&view=online&type=member',
					'loginfield':'username',
					'username':username,
					'password':password,
					'questionid':'0',
			...
			login = self.conn.post(url+'/member.php?mod=logging&action=login&loginsubmit=yes&frommessage&loginhash=Lilog&inajax=1',headers=self.headers,data=data)
			if login.status_code == 200:
				...
				if msglist:
					...
					if '欢迎您回来' in msgstr:
						...
						print(url,' 登录失败！错误信息：',msgstr)
						errormsg = time.strftime('%Y-%m-%d %H:%M:%S')+' '+url+'登录失败！错误信息：'+msgstr+"\r\r"
				else:
					print(url,' 登录失败！')
				...
				print(url,' 登陆失败！错误信息：',login.text)
				errormsg = time.strftime('%Y-%m-%d %H:%M:%S')+' '+url+'登录失败！错误信息：'+login.text+"\r\r"
	...
	def monitoring(self):
		...
		#日志
		logfile = self.url.replace('://','-')+'.log'
		log = open('log/'+logfile,'a',encoding='utf8')
		
		adminGroupList = self.getAdminGroup()#获取管理组 返回一个列表
		...
		while True:
			...
			while True:
				try:
					monitoringurl = self.url+"/home.php?mod=space&do=friend&view=online&type=member&page="+str(page)
					html = self.conn.get(monitoringurl,headers=self.headers)
				except Exception as e:
					print(self.url+monitoringurl,' 访问出错')
			...
			for row in userListId:
				isAdmin = self.isAdmin(adminGroupList,self.url,row)#检测是否是管理员
				if isAdmin:
					print('%s uid \033[0;31m %s \033[0m 用户为 %s 不做任何操作'%(self.url,row,isAdmin,))
					continue
				else:


					print(isAdmin)
			print(self.url+'第 \033[0;31m %d \033[0m 次执行完毕'%(number,))
			number += 1
			
	def isAdmin(self,adminGroupList,url,uid):
		try:
			geturl = url+"/home.php?mod=space&uid="+uid+"&do=profile&from=space"
			usergrouphtml = self.conn.get(geturl,headers=self.headers)
	...
	def getAdminGroup(self):
		"""获取管理组"""
		print(self.url,' 获取管理组')
		try:
			usergroupurl = self.url+"/home.php?mod=spacecp&ac=usergroup"
			html = usergroup = self.conn.get(usergroupurl,headers=self.headers)
